...
Narrative objects are classes that represent an event in your application,
it is through this object that we define meaningful context about the event.
Think of it like a spec file for the event.
...
1. The class must implement the `BetterWorld\Scribe\Contracts\Narrative` contract. To simplify this you may extend
   the `BetterWorld\Scribe\Narrative` abstract class instead.
...
3. Only the following property types are supported (union and intersection types are ignored):
   ...
   - `BetterWorld\Scribe\Support\Json`: A json object, can be instantiated using a valid json string or json encodable
     array.
...
// If the metadata is composed at runtime, you can call the `withMetadata` method instead.
...
// If the scope is composed at runtime, you can call the `withScopes` method instead.
